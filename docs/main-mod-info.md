...
- modInfo is where most of the basic configuration for the mod is. It contains:
    ...
    - discordName, discordLink: If 你有 a Discord server or other discussion place, you can add a link to it.
...
- maxTickLength(): Returns the maximum tick length, in milliseconds. Only really useful if 你有 something that reduces over time, which long ticks mess up (usually a challenge).
